...
mod recording {
    ...
    /// Times a scope and records it in [`GLOBAL`] on drop (no-op when
    /// `ITE_PROFILE` is unset).
    pub struct Span {
        label: &'static str,
...
mod noop {
    /// Zero-cost stand-in: a unit struct with no `Drop`, so `span()` calls
    /// vanish under optimization. `#[inline(always)]` matters — the binary
    /// crate calls this across the crate boundary, where a non-generic
    /// function would otherwise survive without LTO.
    pub struct Span;
