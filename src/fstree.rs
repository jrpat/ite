...
mod tests {
    ...
    #[test]
    fn scans_with_default_filters_dirs_first_sorted() {
        let dir = fixture();
        let tree = FsTree::scan(dir.path(), false).unwrap();
        // Hidden and ignored files are absent; dirs first, then files, each
        // sorted case-insensitively.
        assert_eq!(
            root_names(&tree),
            ["b-dir", "empty-dir", "a-file.txt", "z-file.txt"]
        );
    }

    #[test]
    fn no_ignore_reveals_hidden_and_ignored() {
        let dir = fixture();
